...
submissions/
  ...
  video/demo/                     the expiry demo itself — a page that runs gateCheck() from the sketch against
                                  MemArkiv, blocks ticking at 2s off the wall clock. index.html · demo.ts ·
                                  narrate.py (neural TTS + subtitles) · record.mjs (the 1080p capture)
  video/LAYAK-expiry.mp4          one real-time take of that page: a 40-second certificate goes green, then red
...
npm run video:demo             # re-record it — neural voiceover, subtitles, one real-time 1080p take
```
