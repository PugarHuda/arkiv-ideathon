...
export class MemArkiv {
    ...
    as(signer) { return new MemArkiv(signer, this.st); }
    ...
    blockToUnix(b) { return this.st.genesisUnix + Number(b) * BLOCK_TIME; }
    nowUnix() { return this.blockToUnix(this.block); }
    // ---- wallet actions (same parameter shapes as the SDK) ----
    cost(payload, attributes, expiresIn) {
    ...
    createEntity(p) {
        if (!Number.isInteger(p.expiresIn) || p.expiresIn <= 0 || p.expiresIn % 2 !== 0)
            throw new InvalidExpirationError(String(p.expiresIn)); // R1
        const seq = ++this.st.seq;
        const key = ("0x" + seq.toString(16).padStart(64, "0"));
        ...
        return { entityKey: key, txHash: ("0x" + "t".repeat(0) + key.slice(2)) };
    }
    own(key) { const r = this.rows.get(key); if (!r)
        throw new Error("no such entity"); if (r.owner !== this.signer)
        throw new NotOwnerError(key); return r; } // R3
    extendEntity(p) {
        const r = this.own(p.entityKey);
        const old = r.expiresAtBlock;
        r.expiresAtBlock = this.block + BigInt(p.expiresIn / BLOCK_TIME);
        r.lastModifiedAtBlock = this.block;
    ...
    deleteEntity(p) { this.own(p.entityKey); this.rows.delete(p.entityKey); return { entityKey: p.entityKey, txHash: p.entityKey }; }
    changeOwnership(p) {
        const r = this.own(p.entityKey);
        const old = r.owner;
        r.owner = p.newOwner; // R5: creator untouched
    ...
    mutateEntities(p) {
        ...
        for (const e of p.extensions ?? [])
            this.own(e.entityKey);
        for (const o of p.ownershipChanges ?? [])
            this.own(o.entityKey);
        for (const d of p.deletes ?? [])
            this.own(d.entityKey);
        for (const c of p.creates ?? [])
            if (c.expiresIn % 2 !== 0 || c.expiresIn <= 0)
                throw new InvalidExpirationError(String(c.expiresIn));
        ...
        return { txHash: "0xbatch", createdEntities, updatedEntities: [], deletedEntities, extendedEntities, ownershipChanges };
    }
    // ---- public query surface (same chain shape as the SDK builder) ----
    select(_fields) { return new MemQuery(this); }
}
function matches(r, p) {
    if (p.type === "and")
        return p.predicates.every(q => matches(r, q));
    if (p.type === "or")
        return p.predicates.some(q => matches(r, q));
    const lp = p;
    const a = r.attributes.find(x => x.key === lp.key);
    if (lp.type === "not")
        return a === undefined; // R8
    if (a === undefined)
        return false;
    const v = a.value;
    switch (lp.type) {
        ...
        default:
            if (typeof v !== "number" || typeof lp.value !== "number")
                return false; // R7: ranges on numerics only
...
export class MemQuery {
    ...
    where(...ps) { this.preds.push(...ps.flat()); return this; }
    createdBy(h) { this._creator = h; return this; }
    ownedBy(h) { this._owner = h; return this; }
    limit(n) { this._limit = Math.min(n, PAGE_MAX); return this; }
    validAtBlock(_b) { this._at = _b; return this; }
    all() {
    ...
    async fetch() {
        const rows = this.all();
        const page = rows.slice(this._offset, this._offset + this._limit);
