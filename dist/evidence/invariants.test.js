...
const INSPECTOR = "0x1111111111111111111111111111111111111111";
const CONTRACTOR = "0x2222222222222222222222222222222222222222";
const BUYER = "0x3333333333333333333333333333333333333333";
const W = (n) => ("0x" + String(n).repeat(40));
const any = (x) => x; // MemArkiv is structurally the surface the sketches use; the SDK's client type is a viem client
// ============================== LAYAK ==============================
test("LAYAK-1: an expired certificate is not returned — with no date filter anywhere", async () => {
    ...
    db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 63_072_000, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-4471" }] });
...
test("LAYAK-2: a FAILED exam writes the statutory record and NO certificate, atomically", async () => {
    ...
    const exams = await db.select().where({ type: "eq", key: "type", value: "exam" }).fetch();
    const certs = await db.select().where({ type: "eq", key: "type", value: "cert" }).fetch();
    assert.equal(exams.entities.length, 1);
    assert.equal(certs.entities.length, 0);
...
test("LAYAK-4: only the owner can extend — and an owner extending a cert is CAUGHT by Q8", async () => {
    const db = new MemArkiv(INSPECTOR);
    await L.recordExamination(any(db), { assetId: "A-1", siteId: "S1", certType: "SLO-angkat", bodyId: "B", examRecordId: "E1", regimeCode: 2, outcomeCode: 0, defectCount: 0, testRatioBps: 12500, reportHash: "0x" });
    const cert = (await db.select().where({ type: "eq", key: "type", value: "cert" }).fetch()).entities[0];
    ...
    db.updateEntity({ entityKey: cert.key, payload: cert.payload, contentType: cert.contentType, expiresIn: 63_072_000,
        attributes: cert.attributes.map(a => a.key === "expiresAtTs" ? { key: a.key, value: a.value + 31_536_000 } : a) });
...
test("LAYAK-5: a live certificate with a lapsed Asset is AMBER, not GREEN", async () => {
    const db = new MemArkiv(INSPECTOR);
    db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 60, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-1" }] });
    await L.recordExamination(any(db), { assetId: "A-1", siteId: "S1", certType: "SLO-angkat", bodyId: "B", examRecordId: "E1", regimeCode: 2, outcomeCode: 0, defectCount: 0, testRatioBps: 12500, reportHash: "0x" });
...
test("LAYAK-6: a Prohibition turns the gate red without touching the certificate (no triggers exist)", async () => {
    const db = new MemArkiv(INSPECTOR);
    db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 63_072_000, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-1" }] });
    await L.recordExamination(any(db), { assetId: "A-1", siteId: "S1", certType: "SLO-angkat", bodyId: "B", examRecordId: "E1", regimeCode: 2, outcomeCode: 0, defectCount: 0, testRatioBps: 12500, reportHash: "0x" });
    const pro = db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 86_400, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "prohibition" }, { key: "assetId", value: "A-1" }] });
...
test("LAYAK-7: resale moves $owner and keeps $creator on every certificate", async () => {
    const db = new MemArkiv(INSPECTOR);
    const asset = db.as(CONTRACTOR).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 63_072_000, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-1" }] });
    ...
    const a = (await db.select().where({ type: "eq", key: "type", value: "asset" }).fetch()).entities[0];
    const e = (await db.select().where({ type: "eq", key: "type", value: "exam" }).fetch()).entities[0];
    assert.equal(a.owner, BUYER);
    assert.equal(a.creator, CONTRACTOR);
    assert.equal(e.creator, INSPECTOR, "the past was never the seller's to leave behind");
});
test("LAYAK-8: .count() is one page — complianceGap sums pages, so 350 assets are 350, not 200", async () => {
    ...
    for (let i = 0; i < 350; i++)
        db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 63_072_000, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-" + i }, { key: "siteId", value: "S1" }] });
    const naive = await db.select().where({ type: "eq", key: "type", value: "asset" }).limit(200).count();
    assert.equal(naive, 200, "the naive count the first draft relied on");
    const gap = await L.complianceGap(any(db), "S1", "SLO-angkat");
    assert.equal(gap.assets, 350);
    assert.equal(gap.gap, 350);
});
...
// ============================== SELISIH ==============================
const snap = (round, hf, tier = 0) => ({ market: "aave-v3-eth-wsteth", round, blockNumber: 20_459_000, observedTs: 1_722_800_000, priceE8: 213_928_000_000, healthFactorBps: hf, totalDebtE6: 1, collateralE6: 1, atRiskCount: 0, deviationBps: 0, severityTier: tier, sourceHash: "0x", salt: "s" });
test("SELISIH-1: divergence board returns a SET per round, one row per witness, outlier attributable by creator", async () => {
    ...
    for (const [w, hf] of [[1, 10420], [2, 10420], [3, 10420], [4, 9980], [5, 10420]]) {
        await S.reveal(any(db.as(W(w))), snap(812, hf, w === 4 ? 4 : 0), []);
        db.as(W(w)).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 604_800, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "witness" }, { key: "market", value: "aave-v3-eth-wsteth" }] });
    }
    db.as(W(7)).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 604_800, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "witness" }, { key: "market", value: "aave-v3-eth-wsteth" }] });
    const d = await S.divergence(any(db), "aave-v3-eth-wsteth", 812);
    assert.equal(d.rows.length, 5);
    assert.equal(d.median, 10420);
    assert.equal(d.missing, 1, "6 registered, 5 reported → w7 is the interesting row");
...
test("SELISIH-2: a correction is a NEW entity; the board shows originals only via not(supersedesRound)", async () => {
    ...
    assert.equal(d.median, 10420, "the original is never silently edited; the correction sits beside it");
    const all = await db.select().where({ type: "eq", key: "type", value: "snapshot" }).fetch();
...
test("SELISIH-3: only the owner may extend — a disputant CANNOT preserve a witness's reading; a pin can", async () => {
    ...
    const original = (await db.select().where({ type: "eq", key: "type", value: "snapshot" }).fetch()).entities[0];
    ...
    const snaps = await db.select().where({ type: "eq", key: "type", value: "snapshot" }).fetch();
    const pins = await db.select().where({ type: "eq", key: "type", value: "pin" }).fetch();
    assert.equal(snaps.entities.length, 0, "the witness's reading left the query surface");
    assert.equal(pins.entities.length, 1);
    assert.equal(pins.entities[0].creator, W(9), "the pin is the reader's own entity, funded by the reader, carrying the original tx hash");
...
test("SELISIH-4: conviction is a receipt — cost in ArkivEntityCreated scales with the funded lifetime", async () => {
    ...
    const [c3, c90] = db.events.filter(e => e.name === "ArkivEntityCreated").map(e => e.cost);
...
test("SELISIH-5: pinQueue lists readings that will lapse BEFORE the dispute deadline", async () => {
    ...
    assert.equal(q[0].attributes.find(a => a.key === "round")?.value, 812);
});
test("SELISIH-6: silence is an event — a witness that stops renewing emits ArkivEntityExpired under its own key", async () => {
    ...
    const reg = db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 604_800, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "witness" }, { key: "market", value: "m" }] });
    await S.heartbeat(any(db), reg.entityKey);
    db.advanceSeconds(604_800 - 100);
    assert.equal(db.events.filter(e => e.name === "ArkivEntityExpired").length, 0, "renewed → still live");
    db.advanceSeconds(200);
    const ev = db.events.find(e => e.name === "ArkivEntityExpired");
...
test("SELISIH-7: track record uses createdBy natively — no mirrored witness attribute exists in the schema", async () => {
    const db = new MemArkiv(W(4));
    await S.reveal(any(db), snap(1, 9000, 4), []);
    await S.reveal(any(db), snap(2, 9000, 3), []);
    await S.reveal(any(db), snap(3, 10420, 0), []);
    db.as(W(8)).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 86_400, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "resolution" }, { key: "vindicatedWitness", value: W(4) }] });
    const tr = await S.trackRecord(any(db), W(4));
    assert.deepEqual(tr, { broke: 2, vindicated: 1 });
    const s = (await db.select().where({ type: "eq", key: "type", value: "snapshot" }).fetch()).entities[0];
...
test("SELISIH-8: RosterEpoch answers 'who was expected' after registrations have expired", async () => {
    ...
    for (const w of [1, 2, 3])
        db.as(W(w)).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 604_800, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "witness" }, { key: "market", value: "m" }] });
    ...
    const live = await db.select().where({ type: "eq", key: "type", value: "witness" }).count();
    assert.equal(live, 0, "querying live registrations now answers WRONG with total confidence");
    const epoch = (await db.select().where({ type: "eq", key: "type", value: "roster" }, { type: "lte", key: "roundFrom", value: 812 }, { type: "gte", key: "roundTo", value: 812 }).fetch()).entities[0];
