...
#
# Run:  python submissions/video/demo/narrate.py
# Needs: edge-tts, ffmpeg/ffprobe on PATH.  Writes: vo/*.mp3, vo.mp3, narration.json, LAYAK-demo.srt/.vtt
...
def main() -> None:
    ...
    subprocess.run(["ffmpeg", "-y", "-v", "error", "-f", "concat", "-safe", "0", "-i", str(listing),
                    "-c:a", "aac", "-b:a", "128k", str(HERE / "vo.m4a")], check=True)

    (HERE / "narration.json").write_text(json.dumps({"voice": VOICE, "rate": RATE, "gap": GAP,
                                                     "total": round(total, 3), "cues": cues}, indent=1), encoding="utf-8")

    srt = "\n".join(f"{i}\n{ts(c['start'], ',')} --> {ts(c['end'], ',')}\n{c['text']}\n"
                    for i, c in enumerate(cues, 1))
    (HERE / "LAYAK-demo.srt").write_text(srt, encoding="utf-8")
    vtt = "WEBVTT\n\n" + "\n".join(f"{ts(c['start'], '.')} --> {ts(c['end'], '.')}\n{c['text']}\n" for c in cues)
    (HERE / "LAYAK-demo.vtt").write_text(vtt, encoding="utf-8")

    print(f"\nnarration {total:.2f}s over {len(cues)} lines -> narration.json, vo.m4a, .srt, .vtt")
    if total > 75:
        print("WARNING: narration is long for a 60-second demo", file=sys.stderr)
