...
test("LAYAK-8: .count() is one page — complianceGap sums pages, so 350 assets are 350, not 200", async () => {
  ...
  for (let i = 0; i < 350; i++) db.createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 63_072_000, attributes: [{ key: "project", value: "layak" }, { key: "type", value: "asset" }, { key: "assetId", value: "A-" + i }, { key: "siteId", value: "S1" }] });
...
test("SELISIH-8: RosterEpoch answers 'who was expected' after registrations have expired", async () => {
  const db = new MemArkiv(W(1));
  for (const w of [1, 2, 3]) db.as(W(w)).createEntity({ payload: new Uint8Array(0), contentType: "application/json", expiresIn: 604_800, attributes: [{ key: "project", value: "selisih" }, { key: "type", value: "witness" }, { key: "market", value: "m" }] });
