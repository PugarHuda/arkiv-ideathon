...
**Watch the claim run** — one unedited real-time take, no cuts: https://pugarhuda.github.io/arkiv-ideathon/submissions/video/layak.html — a certificate written with a 40-second lifetime goes green, then red, while the query on screen never changes and no date is ever compared. The gate check is `gateCheck()` imported unmodified from the sketch, running against the same executable spec the invariants use; the page is at https://pugarhuda.github.io/arkiv-ideathon/submissions/video/demo/ if you would rather watch it happen yourself.
